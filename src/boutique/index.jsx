...
import GridElement, { Partenaire, SociauxItem } from "../../composants/gridElement";
import { TitleBar } from "../home/home";
import Grid, { Gridtemplate_3 } from "../../composants/grid";
import Vide from "../../composants/vide";
import { MdiBasketPlus, PlayIcon } from "../../composants/icons";
...
import Store from "../context/testeStore";
...
export default function Boutique(){
   ...
   async function getProduct(){
        ...
        const getSnapshot = await getDocs(docRef)
        const arr = []

        if(!getSnapshot.empty){
            getSnapshot.forEach(Element => {
                arr.push({
                    id : Element.id,
                    ...Element.data()
                })
   ...
   createEffect(() => {
    ...
    return(
        <div>
            <Container>
                <Vide />
                <TitleBar titre={"Boutique"} />
                <Gridtemplate_3>
                   <For each = {store}>
                        {
                            (element) => <ProductItems images={element.image} prix={element.price} name={element.name} id={element.id} data = {element} />
                        }
                   </For>
                </Gridtemplate_3>
                ...
                <Grid>
                    ...
                    <div style={{
                            ...
                            <span style={{"font-size" : '32px', "font-weight" : '600', width : "350px"}}>Nos Expert et nos partenaires sont de plus en plus proche de vous </span>
                        </div>
                    </GridElement>
                </Grid>
...
function ProductItems({images, name, prix, id, data}){
    const Prix = parseInt(prix)
    const Price = new Intl.NumberFormat('fr-FR', { style : 'currency', currency : 'XAF'}).format(Prix)


    return(
        <A href={"/shop/"+id} class={styles.produit} >
            <div class = {styles.image}>
                <img src={images} alt=""  class={styles.img}/>
            </div>
            <div>
                <div class={styles.name}>{name}</div>
                <div class={styles.details}>
                    <div>
                        <div class={styles.titreProduit}>Prix</div>
                        <div>{Price}</div>
                    </div>
                    <div>
                        <button>
